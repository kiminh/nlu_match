...
class TaggingConverter(object):
  ...
  def __init__(self, phrase_vocabulary, do_swap=False, tokenizer=None):
    ...
    # Maximum number of tokens in an added phrase (inferred from the vocabulary).
  ...
  def compute_tags(self, task, target):
    ...
    """
    target_tokens = self.tokenizer.tokenize(target) # utils.get_token_list(target.lower())
    tags = self._compute_tags_fixed_order(task.source_tokens, target_tokens, task.labels_mask)
    # If conversion fails, try to obtain the target after swapping the source order.
    if not tags and self._do_swap and len(task.sources) == 2:
      swapped_task = tagging.EditingTask(task.sources[::-1])
      tags = self._compute_tags_fixed_order(swapped_task.source_tokens,
                                            target_tokens, task.labels_mask)  # TODO TODO  not support now
      if tags:
  ...
  def _compute_tags_fixed_order(self, source_tokens, target_tokens, labels_mask):
    ...
    start_token_idx = 0
    while labels_mask[start_token_idx] == 0:
      start_token_idx += 1
    # print("start_token_idx=",start_token_idx, source_tokens[start_token_idx-1])
    source_tokens = source_tokens[start_token_idx:]
    # print(len(labels_mask), "labels_mask:", labels_mask)
    # print(len(source_tokens), "source_tokens:", source_tokens)
    # print(curLine(), len(target_tokens), "target_tokens:", target_tokens)
    ...
    while target_token_idx < len(target_tokens):
      ...
      if source_token_idx >= len(tags):
        break
    # print(curLine(), len(tags), "tags:", target_token_idx, len(target_tokens))
    ...
    if target_token_idx >= len(target_tokens): # all target tokens have been consumed
      # for stoken, tag in zip(source_tokens, tags):
      #   print(curLine(), stoken, tag.tag_type, tag.added_phrase)
      return [tagging.Tag('DELETE')] * start_token_idx + tags
    return [] # TODO   不能转化
