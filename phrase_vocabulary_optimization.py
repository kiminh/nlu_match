...
def _get_added_phrases(source_tokens:list, target_tokens:list) -> Sequence[Text]:
...
def _added_token_counts(data_iterator, try_swapping, max_input_examples=10000, tokenizer=None):
  ...
  for sources, target in data_iterator:
    ...
    if num_examples >= max_input_examples:
      break
    source_merge = sep_str.join(sources)
    source_tokens = tokenizer.tokenize(source_merge)
    target_tokens = tokenizer.tokenize(target)
    if sep_str not in source_tokens:
        print(curLine(), len(source_tokens), "ignore source_tokens:", source_tokens)
        continue
    if len(source_tokens) > max_seq_length:
        print(curLine(), "max_seq_length=%d, len(source_tokens)=%d, source_tokens:" %
              (max_seq_length, len(source_tokens)), source_tokens)
        max_seq_length = len(source_tokens)
    ...
    if try_swapping and len(sources) == 2:
      source_merge = sep_str.join(sources[::-1])
      # print(curLine(), len(source_merge), "source_merge:", source_merge)
      source_tokens = tokenizer.tokenize(source_merge)
      added_phrases_swap = _get_added_phrases(source_tokens, target_tokens)
...
def main(argv):
  ...
  phrase_counter, all_added_phrases, max_seq_length = _added_token_counts(
      data_iterator, FLAGS.enable_swap_tag, FLAGS.max_input_examples, tokenizer=tokenizer)
  ...
  with tf.gfile.GFile(FLAGS.output_file, 'w') as writer:
    with tf.gfile.GFile(statistics_file, 'w') as stats_writer:
