...
class EditingTask(object):
  ...
  def __init__(self, sources, location=None, tokenizer=None, sep_mark=None):
    ...
    question, passage = sources
    question = normal_transformer(question)
    self.passage = normal_transformer(passage)
    self.token_index_map = {}  # 为处理生成的ＵＮＫ

    tokens_question = tokenizer.tokenize(question)
    tokens_passage = tokenizer.tokenize(self.passage)
    previous_id = 0
    for tokenizer_id, t in enumerate(tokens_passage):
      if tokenizer_id > 0 and "UNK" in tokens_passage[tokenizer_id-1]:
        if t in self.passage[previous_id:]:
          previous_id = previous_id + self.passage[previous_id:].index(t)
    ...
    # print(curLine(), len(tokens_passage), "tokens_passage:", tokens_passage)
    # print(curLine(), len(tokens_question), "tokens_question:", tokens_question)
    # print(curLine(), len(self.token_index_map), "self.token_index_map:", self.token_index_map)
    # Tokens of the source texts concatenated into a single list.
    self.source_tokens = tokens_question + [sep_mark]
    self.segment_ids = [0] * (len(self.source_tokens)) # after cls to first sep
    # The indices of the first tokens of each source text.
    self.first_tokens = [0, len(self.source_tokens)]
    self.source_tokens.extend(tokens_passage)  # 最后一个'[SEP]'会在其他地方加上
    self.segment_ids.extend( [1]*(len(tokens_passage))) # after first sep, last sep
    self.labels_mask = [0] * (len(tokens_question) + 1) + [1] * (len(tokens_passage))
    assert len(self.segment_ids) == len(self.source_tokens)
  ...
  def _realize_sequence(self, tokens, tags):
    ...
    for index, (token, tag) in enumerate(zip(tokens, tags)):
      ...
      if tag.added_phrase and (loc == "0" or index==0 or (index>0 and self.location[index-1]=="0")):  # TODO
        if not (tag.added_phrase.startswith("##", 0, 2) and len(output_tokens)>0):
      ...
      if tag.tag_type in (TagType.KEEP, TagType.SWAP) or loc == "1":  # TODO 根据需要修改代码,location为"1"的位置不能被删除, 但目前是可以插入的
        if token.startswith("##", 0, 2) and len(output_tokens) > 0:
          ...
          token = self.passage[previoud_id:next_previoud_id]
  ...
  def realize_output(self, tags):
    ...
    # print(curLine(), len(outputs), "outputs:", outputs)
